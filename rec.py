...
def data_collection():
    ...
    os.mkdir(f"Participants/{name}")
    os.chdir(f"Participants/{name}")
    os.mkdir("screenshots")
    start = time.time()
    last_mouse_record_time = 0
    rec_time = 0
    while True:
        ctime = time.time()
        if ctime - last_mouse_record_time >= 0.2:
            mouse_x, mouse_y = pyautogui.position()
            mouse_data.append(['Mouse Position', mouse_x, mouse_y])
            last_mouse_record_time = ctime
            x, y = gazetracker.get_gaze_position()
            gaze_data.append([time.time() - cctime, x, y])
        if (time.time() - rec_time) % 10 < 0.2:
            rec_time = time.time()
            write_to_files()
